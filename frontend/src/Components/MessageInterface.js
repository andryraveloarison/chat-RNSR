...
const MessageInterface = () => {
  ...
  useEffect(() => {
    ...
    const getInfoUserReceiver = async () => {
        try {
            ...
            setUserReceiver((prevItem) => ({
                ...prevItem,
                usernameReceiver: rep.data.data.name,
                profileReceiver: `https://i.pravatar.cc/150?img=${rep.data.data.name}`,
    ...
    const getInfoUser = async () => {
        try {
            ...
            setUser((prevItem) => ({
                ...
                profile: `https://i.pravatar.cc/150?img=${rep.data.data.id}`
            }));
    ...
    const getMessageUser = async() => {
      try {
        ...
        const allmessage = rep.data.sort((a, b) => 
  ...
  useEffect(() => {
    ...
    socket.on('new_message', (msg) => {
      // console.log(msg.rows[0].receiverId ,'===', user.id.toString() ,'&&', msg.rows[0].senderId ,'===', idUserReceiver.id.toString());
      if  (
            (msg.receiverId === user.id.toString() && msg.senderId === idUserReceiver.id.toString()) ||
            (msg.receiverId === idUserReceiver.id.toString() && msg.senderId === user.id.toString())
          ) {
        setMessages(prevMessages => [...prevMessages, msg]);
      }
    });

    socket.on('new_connection', async (newUser) => {

      if(idUserReceiver.id.toString() === newUser.id){
        setUserReceiver({ ...userReceiver, status: 'Connected' })
      }

  })

  socket.on('new_deconnection', async (userId) => {
    if(idUserReceiver.id.toString() === userId){
      setUserReceiver({ ...userReceiver, status: 'Disconnected' })
    }
  })
        ...
        <div className="flex flex-col h-[90vh] bg-[#101010dc] rounded-lg p-4 w-[500px] text-white">
            ...
            <div className='flex flex-row justify-between items-center border-b border-gray-600 pb-2'>
               <div className='flex flex-row justify-start gap-3 items-center'>
                    ...
                    <div>
                        ...
                        <div className='text-[10px] text-gray-500'>{userReceiver.status}</div>
                    </div>
               </div>
            ...
            <div className="flex-1 p-4 overflow-y-auto">
                ...
                <div
                    ...
                    msg.receiverId === idUserReceiver.id ? 'justify-end' : 'justify-start'
                    } mb-4`}
                >
                    <div className={`${msg.receiverId === idUserReceiver.id ? ` hidden` : ` block`}`}><img src={`${userReceiver.profileReceiver}`}  alt="" className={`w-8 h-8 rounded-full`}/></div>
                    <div className='flex flex-col justify-start gap-0'>
                      <div
                      className={`w-[200px] text-[12px] px-4 py-2 rounded-lg ${
                        msg.receiverId === idUserReceiver.id ? 'bg-[#2563EB] text-white' : 'bg-gray-300 text-gray-900'
                      }`}
                      >
                      {msg.content}
                      </div>
